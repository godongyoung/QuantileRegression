...
for(sim_idx in start.idx:end.idx){
    ...
    {if (type=='N'){
        ...
        if (type=='locshitJASA'){
            beta.true=c(0,beta[2]+0.5*qnorm(p0,0,1))
        }          
        if (type=='MixN'){
            beta.true=beta
        }}
    ...
    toc()
    save.image(file=sprintf('../debugging/GAL_%s_%s_%s.RData',type,p0,sim_idx))
}
...
#   load(file=sprintf('../debugging/GAL_%s_%s_%s.RData',type,p0,sim_idx))

...
NQR_w_MME_Gamma=function(y,W1,W2,p0,inp.min,inp.max,multiply_c=2,inp.version=3,N.Knots=30,Knots.direct=NA){
  ...
  log.likeli.x=function(X.1t){ ### This can be changed with individual updates!
    ...
    term3 = sum(dnorm(W2,mean = gamma.t[1]+gamma.t[2]*X.1t,sd = sqrt(sigma2_22.t),log = T))
  ...
  log.likeli.x.i=function(X.1t){ ### This can be changed with individual updates!
    ...
    term3 = (dnorm(W2,mean = gamma.t[1]+gamma.t[2]*X.1t,sd = sqrt(sigma2_22.t),log = T))
  ...
  gamma.t=rmvnorm(1,c(0,0),diag(2))
  sigma2_xx.t=10#runif(1)
  sigma2_11.t=10#runif(1)
  sigma2_22.t=10#runif(1)
  mux.t=rnorm(1,0,sqrt(100))
  # X.1t=cbind(1,W1)%*%t(alpha.t) #Let's start with 21, obs data, This was wrong!
  # X.1t=W2
  X.1t=(W2-gamma[1])/gamma[2]
  ...
  #N for gamma
  pr_mean_gamma=rep(0,2)
  pr_var_gamma=var_param*diag(2)
  ...
  gamma_trace=matrix(NA,nrow=nmcmc,ncol=2)
  ...
  for(iter in 1:niter){
    ...
    post_b2=prior_b2+1/2*sum((W2-(X.t%*%t(gamma.t)))^2)
    ...
    #sample gamma-----------------------------------------------------------------
    post_V_gamma = solve(t(X.t)%*%X.t/sigma2_22.t + solve(pr_var_gamma))
    post_M_gamma = post_V_gamma %*% (t(X.t)%*%W2/sigma2_22.t + solve(pr_var_gamma)%*%pr_mean_gamma)
    gamma.t = rmvnorm(n = 1, mu = post_M_gamma,sigma = post_V_gamma)        
    ...
    if((iter > nburn) & (iter %% nthin== 0) ){
      ...
      gamma_trace[thinned_idx,]=gamma.t
      X_trace[thinned_idx,] = X.1t
    }
  }
  toc()
...
NQR_w_3ME=function(y,W1,W2,W3,p0,inp.min,inp.max,multiply_c=2,inp.version=3,N.Knots=30,Knots.direct=NA,inp.sigma.g=0.01){
  # Function --------------------------------------------------------------------------------
  log.likeli.g=function(g.t, lambda.t,X.1t){
    y.est=smooth.y(tau.i,g.t,X.1t,version=inp.version)
    ui=y-y.est
  ...
  log.likeli.x=function(X.1t){ ### This can be changed with individual updates!
    ...
    term2 = sum(dnorm(W1,mean = alpha.t[1]+alpha.t[2]*X.1t,sd = sqrt(sigma2_11.t),log = T))
    term3 = sum(dnorm(W2,mean = X.1t,sd = sqrt(sigma2_22.t),log = T))
    term3.5 = sum(dnorm(W3,mean = beta.t[1]+beta.t[2]*X.1t,sd = sqrt(sigma2_33.t),log = T))
    term4 = sum(dnorm(X.1t,mean = mux.t,sd = sqrt(sigma2_xx.t),log = T))
    return (term1 + term2 + term3 + term3.5 + term4)
  }
  ...
  log.likeli.x.i=function(X.1t){ ### This can be changed with individual updates!
    ...
    term2 = (dnorm(W1,mean = alpha.t[1]+alpha.t[2]*X.1t,sd = sqrt(sigma2_11.t),log = T))
    term3 = (dnorm(W2,mean = X.1t,sd = sqrt(sigma2_22.t),log = T))
    term3.5 = (dnorm(W3,mean = beta.t[1]+beta.t[2]*X.1t,sd = sqrt(sigma2_33.t),log = T))
    term4 = (dnorm(X.1t,mean = mux.t,sd = sqrt(sigma2_xx.t),log = T))
    return (term1 + term2 + term3 + term3.5 + term4)
  }
  # Set Defaults --------------------------------------------------------------------------------
  ...
  for(iter in 1:niter){
    ...
    #sample sigma2_33-----------------------------------------------------------------
    post_a3=0.5*n+prior_a3
    post_b3=prior_b3+1/2*sum((W3-(X.t%*%t(beta.t)))^2)
    sigma2_33.t=rinvgamma(1,post_a3,post_b3)
    stopifnot(sigma2_33.t>0)
    ...
    #sample beta-----------------------------------------------------------------
    post_V_beta = solve(t(X.t)%*%X.t/sigma2_33.t + solve(pr_var_beta))
    post_M_beta = post_V_beta %*% (t(X.t)%*%W3/sigma2_33.t + solve(pr_var_beta)%*%pr_mean_beta)
    beta.t = rmvnorm(n = 1, mu = post_M_beta,sigma = post_V_beta)
    ...
    if((iter > nburn) & (iter %% nthin== 0) ){
      ...
      sigma2_11_trace[thinned_idx]=sigma2_11.t
      sigma2_22_trace[thinned_idx]=sigma2_22.t
      sigma2_33_trace[thinned_idx]=sigma2_33.t
      mux_trace[thinned_idx]=mux.t
      alpha_trace[thinned_idx,]=alpha.t
  ...
  res_list[['beta_trace']]=beta_trace
  res_list[['mux_trace']]=mux_trace
  res_list[['sigma2_33_trace']]=sigma2_33_trace
  res_list[['sigma2_22_trace']]=sigma2_22_trace
  res_list[['sigma2_11_trace']]=sigma2_11_trace
  res_list[['sigma2_xx_trace']]=sigma2_xx_trace
  res_list[['Knots']]=tau.i
...
NQR_w_4ME=function(y,W1,W2,W3,W4,p0,inp.min,inp.max,multiply_c=2,inp.version=3,N.Knots=30,Knots.direct=NA,inp.sigma.g=0.01){
  # Function --------------------------------------------------------------------------------
  log.likeli.g=function(g.t, lambda.t,X.1t){
    y.est=smooth.y(tau.i,g.t,X.1t,version=inp.version)
    ui=y-y.est
  ...
  log.likeli.x=function(X.1t){ ### This can be changed with individual updates!
    ...
    term3 = sum(dnorm(W2,mean = X.1t,sd = sqrt(sigma2_22.t),log = T))
    term3.5 = sum(dnorm(W3,mean = beta.t[1]+beta.t[2]*X.1t,sd = sqrt(sigma2_33.t),log = T))
    term3.6 = sum(dnorm(W4,mean = theta.t[1]+theta.t[2]*X.1t,sd = sqrt(sigma2_44.t),log = T))
    term4 = sum(dnorm(X.1t,mean = mux.t,sd = sqrt(sigma2_xx.t),log = T))
    return (term1 + term2 + term3 + term3.5 + term3.6 + term4)
  }
  ...
  log.likeli.x.i=function(X.1t){ ### This can be changed with individual updates!
    ...
    term3 = (dnorm(W2,mean = X.1t,sd = sqrt(sigma2_22.t),log = T))
    term3.5 = (dnorm(W3,mean = beta.t[1]+beta.t[2]*X.1t,sd = sqrt(sigma2_33.t),log = T))
    term3.6 = (dnorm(W4,mean = theta.t[1]+theta.t[2]*X.1t,sd = sqrt(sigma2_44.t),log = T))
    term4 = (dnorm(X.1t,mean = mux.t,sd = sqrt(sigma2_xx.t),log = T))
    return (term1 + term2 + term3 + term3.5 + term3.6 + term4)
  }
  # Set Defaults --------------------------------------------------------------------------------
  ...
  alpha.t=rmvnorm(1,c(0,0),diag(2))
  beta.t=rmvnorm(1,c(0,0),diag(2))
  theta.t=rmvnorm(1,c(0,0),diag(2))
  sigma2_xx.t=10#runif(1)
  sigma2_11.t=10#runif(1)
  sigma2_22.t=10#runif(1)
  sigma2_33.t=10#runif(1)
  sigma2_44.t=10#runif(1)
  mux.t=rnorm(1,0,sqrt(100))
  # X.1t=cbind(1,W1)%*%t(alpha.t) #Let's start with 21, obs data
  X.1t=W2
  # X.1t=(cbind(1,W1)%*%t(alpha.t) +W2)/2
  ...
  pr_var_beta=var_param*diag(2)
  #N for theta
  pr_mean_theta=rep(0,2)
  pr_var_theta=var_param*diag(2)
  ...
  sigma2_22_trace=rep(NA,nmcmc)
  sigma2_33_trace=rep(NA,nmcmc)
  sigma2_44_trace=rep(NA,nmcmc)
  mux_trace=rep(NA,nmcmc)
  alpha_trace=matrix(NA,nrow=nmcmc,ncol=2)
  beta_trace=matrix(NA,nrow=nmcmc,ncol=2)
  theta_trace=matrix(NA,nrow=nmcmc,ncol=2)
  X_trace=matrix(NA,nrow=nmcmc,ncol=n)   
  ...
  for(iter in 1:niter){
    ...
    #sample sigma2_44-----------------------------------------------------------------
    post_a4=0.5*n+prior_a4
    post_b4=prior_b4+1/2*sum((W4-(X.t%*%t(theta.t)))^2)
    sigma2_44.t=rinvgamma(1,post_a4,post_b4)
    stopifnot(sigma2_44.t>0)
    ...
    #sample theta-----------------------------------------------------------------
    post_V_theta = solve(t(X.t)%*%X.t/sigma2_44.t + solve(pr_var_theta))
    post_M_theta = post_V_theta %*% (t(X.t)%*%W4/sigma2_44.t + solve(pr_var_theta)%*%pr_mean_theta)
    theta.t = rmvnorm(n = 1, mu = post_M_theta,sigma = post_V_theta)
    ...
    if((iter > nburn) & (iter %% nthin== 0) ){
      ...
      sigma2_22_trace[thinned_idx]=sigma2_22.t
      sigma2_33_trace[thinned_idx]=sigma2_33.t
      sigma2_44_trace[thinned_idx]=sigma2_44.t
      mux_trace[thinned_idx]=mux.t
      alpha_trace[thinned_idx,]=alpha.t
      beta_trace[thinned_idx,]=beta.t
      theta_trace[thinned_idx,]=theta.t
      X_trace[thinned_idx,] = X.1t
    }
  }
  toc()
  ...
  res_list[['alpha_trace']]=alpha_trace
  res_list[['beta_trace']]=beta_trace
  res_list[['theta_trace']]=theta_trace
  res_list[['mux_trace']]=mux_trace
  res_list[['sigma2_44_trace']]=sigma2_44_trace
  res_list[['sigma2_33_trace']]=sigma2_33_trace
  res_list[['sigma2_22_trace']]=sigma2_22_trace
  res_list[['sigma2_11_trace']]=sigma2_11_trace
  res_list[['sigma2_xx_trace']]=sigma2_xx_trace
...
NQR_w_SME=function(y,W1,W2,p0,inp.min,inp.max,multiply_c=2,inp.version=3,N.Knots=30,Knots.direct=NA,inp.sigma.g=0.01,g0.cheat.fname = NA){
  # Function --------------------------------------------------------------------------------
  log.likeli.g=function(g.t, lambda.t,X.1t){
    y.est=smooth.y(tau.i,g.t,X.1t,version=inp.version)
    ui=y-y.est
  ...
  # cheat X0 if g0.cheat.fname is given
  if(!is.na(g0.cheat.fname)){
    load(g0.cheat.fname)
    if(is.null(dim(NQR_res$X_trace))){
      X.1t = NQR_res$X_trace
    }
    else{
      X.1t = colMeans(NQR_res$X_trace)
    }
  }
  # X.1t=(cbind(1,W1)%*%t(alpha.t) +W2)/2
  # X.1t=rnorm(n,0,100)
  ...
  # cheat g0 if g0.cheat.fname is given
  if(!is.na(g0.cheat.fname)){
    load(g0.cheat.fname)
    g0 = colMeans(NQR_res$g_trace)
  }

...
for(p0 in p0_list){
  ...
  for(ii in 1:nmax){
    n=100
    si=rtruncnorm(n = n,a = 0,b = Inf,mean = 0,sd = 1)
    zi=rexp(n,1)
    ui=rnorm(n,0,1)
    A=(1-2*p0)/(p0*(1-p0))
    B=2/(p0*(1-p0))
    ...
    X=cbind(1,x1i)
    sigma=3
    y=X%*%beta+sigma*hyperparam.alpha*si + sigma*A*zi+sigma*sqrt(B)*sqrt(zi)*ui
...
for(p0 in names(res_list)){
  p0_mat=res_list[[p0]]  
  for(to_see in names(p0_mat)){
    print(c(p0,to_see))
    p0_mat[[to_see]][is.na(p0_mat[[to_see]])]='|'
    print(p0_mat[[to_see]])
  }
}

# save.image(file='../debugging/Debugging_w_GAL_example_data.RData')
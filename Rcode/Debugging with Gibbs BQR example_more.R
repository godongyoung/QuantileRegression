...
for (type in type_list){
    ...
    for(p0 in c(0.1,0.5,0.9)){
        ...
        beta_save4=matrix(NA,ncol=P,nrow=nmax)
        for(idx in 1:nmax){
            tryCatch({
                # make data ------------------------------
                set.seed(idx)
                ...
                X=cbind(1,x2,x3)
                stopifnot(P==dim(X)[2])
                ei=gen_error(type=type,n=n,x2=x3)
                beta=c(1,1,1)
                y=X%*%beta+ei
                ...
                beta_save4[idx,]=colMeans(ALD_res[['beta_trace']])
...
# save.image(file='../debugging/Debugging_w_GibbsBQR_data_more.RData')
# load(file='../debugging/Debugging_w_GibbsBQR_data_more.RData')
...
for (type in names(total_df)){
    ...
    for (qversion in names(type_df)){
        ...
        df=data.frame(matrix(NA,nrow=length(names(q_type_df)),ncol=((dim(X)[2]*3+2))),row.names = names(q_type_df))
        cnt=1

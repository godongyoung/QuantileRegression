...
alpha.t=matrix(alpha,ncol=2) # Starts with cheating value
gamma.t=matrix(gamma,ncol=2) # Starts with cheating value
...
# X.1t=W2
# X.1t=(W2-(gamma+rnorm(2))[1])/(gamma+rnorm(2))[2]
# X.1t=((W2-(gamma+rnorm(2))[1])/(gamma+rnorm(2))[2]+(W1-(alpha+rnorm(2))[1])/(alpha+rnorm(2))[2])/2
X.1t=(W1-(alpha+rnorm(2))[1])/(alpha+rnorm(2))[2]
...
#--------------------------------------------------------------------------------------------------------------------------------
plot(X[,2],colMeans(X_trace[1:5000,],na.rm = T));abline(0,1)
ts.plot(X_trace[,10])

ts.plot(sigma2_22_trace)
hist(sigma2_22_trace[4500:5000],nclass=100)
alpha.est = colMeans(alpha_trace[4500:5000,]);
alpha
gamma.est = colMeans(gamma_trace[4500:5000,]);
gamma

ts.plot(alpha_trace[,1])
ts.plot(alpha_trace[,2])
ts.plot(gamma_trace[,1])
ts.plot(gamma_trace[,2])

g.est=colMeans(g_trace)
plot(X[,2],y)
lines(tau.i,g.est,lwd=3,col=2)
y.p0=2+sin(tau.i)+qnorm(p0,0,inp.sd)
lines(tau.i,y.p0,lwd=2,col=2)

ts.plot(g_trace[,10]);abline(h=y.p0[10])
...
plot(cbind(1,X.est)%*%gamma.est,W2);abline(0,1)
plot(g.est)
